...
export default function ExportPdfPage() {
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      doc.text(`Period: ${dateFrom || 'All'} → ${dateTo}`, 14, 27)
      ...
      statItems.forEach(([label, value], i) => {
        ...
        doc.setFont('helvetica', 'bold')
        doc.setTextColor(
          value.startsWith('-') ? 180 : value.startsWith('+') || (!value.startsWith('-') && !value.startsWith('$-') && label.includes('P/L') && !value.startsWith('$-')) ? 30 : 30,
          value.startsWith('-') ? 30 : value.startsWith('+') ? 120 : 30,
          value.startsWith('-') ? 30 : 30
        )
      ...
      autoTable(doc, {
        startY: y,
        head: [['Date', 'Pair', 'Side', 'Strategy', 'Entry', 'Exit', 'Net P/L', 'Result']],
        body: filtered.map(t => [
        ...
        columnStyles: {
          0: { cellWidth: 18 }, 1: { cellWidth: 20 }, 2: { cellWidth: 12 },
          3: { cellWidth: 30 }, 4: { cellWidth: 22 }, 5: { cellWidth: 22 },
          6: { cellWidth: 18 }, 7: { cellWidth: 14 },
        },
        didDrawCell: (hookData) => {
          if (hookData.column.index === 7 && hookData.section === 'body') {
            const val = String(hookData.cell.text)
            if (val === 'WIN')  hookData.doc.setTextColor(16, 185, 129)
            if (val === 'LOSS') hookData.doc.setTextColor(239, 68, 68)
          }
        },
      })
  ...
  const inp = 'bg-[#1a1a2a] border border-[#2a2a3a] text-gray-200 text-sm px-3 py-2.5 rounded-xl focus:outline-none focus:border-blue-500'
    ...
    <div className="max-w-lg mx-auto p-6 space-y-5">
      ...
      {filtered.length === 0 ? (
        ...
        <button onClick={handleExportPdf} disabled={exporting}
          className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white py-3.5 rounded-xl text-sm font-bold transition-colors">
          {exporting
