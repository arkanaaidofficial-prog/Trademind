...
export default function ReviewsPage() {
  ...
  async function load() {
    ...
    if (!user) return
    const [{ data: rv }, { data: tr }] = await Promise.all([
  ...
  async function handleSave() {
    ...
    // Snapshot stats for this period
    ...
    setSaving(false)
    if (error) { toast.error('Gagal menyimpan review: ' + error.message); return }
    toast.success('Review berhasil disimpan!')
    setShowForm(false)
    setForm({ period_type: 'weekly', period_start: new Date().toISOString().slice(0,10), period_end: new Date().toISOString().slice(0,10), what_worked: '', biggest_mistake: '', main_lesson: '', strategy_continue: '', strategy_stop: '', risk_management_ok: true, improvement_plan: '', overall_rating: 7 })
    load()
  }
